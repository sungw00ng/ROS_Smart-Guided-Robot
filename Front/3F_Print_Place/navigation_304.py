...
class ProfessorCard(QWidget):
    def __init__(self):
        ...
        # image/person_313.png 경로 그대로 사용
        ...
        self.sub_img = QLabel()
        # image/person_313_qr_dbpia.png 경로 그대로 사용
        ...
        # --- 가운데 텍스트 정보 (304호 내용으로 변경) ---
        text = (
            "\n\n\n[304호]\n데이터 통신실\n\n"
            "[주요 시설]\n\n"
            "네트워크 장비, 통신 시스템\n"
            "데이터 분석 및 처리 장비"  # 수업 시간 포맷 유지
        )
        ...
        # --- 오른쪽 이미지 (클릭 가능, 304호 이미지로 변경) ---
        self.right_img = ClickableLabel()
        right_img_path = os.path.join(self.project_root, "image", "304호.png")  # 이미지 경로 304호로 변경
        ...
        if not pixmap_right.isNull():
            ...
            self.right_img.setText("304호 이미지 없음")
            self.right_img.setStyleSheet("color: white; font-size: 30px;")
            print(f"경고: 304호 이미지 파일을 찾을 수 없습니다: {right_img_path}")
        ...
        # 시작하자마자 MP3 재생 (304호 정보 오디오로 변경)
        self.play_info_audio("AI_SIM_mp3/info_304.mp3")  # 오디오 파일명 304로 변경
