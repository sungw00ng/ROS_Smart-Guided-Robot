...
class ProfessorCard(QWidget):
    def __init__(self):
        ...
        # --- 오른쪽 이미지 (클릭 가능, 312호 이미지로 변경) ---
        self.right_img = ClickableLabel()
        right_img_path = os.path.join(self.project_root, "image", "312호.png")  # 이미지 경로 312호로 변경
        ...
        if not pixmap_right.isNull():
            ...
            self.right_img.setText("312호 이미지 없음")
            self.right_img.setStyleSheet("color: white; font-size: 30px;")
            print(f"경고: 312호 이미지 파일을 찾을 수 없습니다: {right_img_path}")
        ...
        # 시작하자마자 MP3 재생 (312호 정보 오디오로 변경)
        self.play_info_audio("AI_SIM_mp3/info_312.mp3")  # 오디오 파일명 312로 변경
