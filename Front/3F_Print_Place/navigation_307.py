...
class ProfessorCard(QWidget):
    def __init__(self):
        ...
        # image/person_307.png 경로로 변경
        main_img_path = os.path.join(self.project_root, "image", "person_307.png")
        ...
        self.sub_img = QLabel()
        # image/person_307_qr_dbpia.png 경로로 변경
        sub_img_path = os.path.join(self.project_root, "image", "person_307_qr_dbpia.png")
        ...
        self.qr_text_label = QLabel("권기태 교수님 Dbpia")  # 텍스트 변경
        ...
        # --- 오른쪽 이미지 (클릭 가능, 307호 이미지로 변경) ---
        self.right_img = ClickableLabel()
        right_img_path = os.path.join(self.project_root, "image", "307호.png")  # 이미지 경로 307호로 변경
        ...
        if not pixmap_right.isNull():
            ...
            self.right_img.setText("307호 이미지 없음")
            self.right_img.setStyleSheet("color: white; font-size: 30px;")
            print(f"경고: 307호 이미지 파일을 찾을 수 없습니다: {right_img_path}")
        ...
        # 시작하자마자 MP3 재생 (307호 정보 오디오로 변경)
        self.play_info_audio("AI_SIM_mp3/info_307.mp3")  # 오디오 파일명 307로 변경
