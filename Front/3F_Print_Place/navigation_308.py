...
class ProfessorCard(QWidget):
    def __init__(self):
        ...
        # image/person_308.png 경로로 변경 (김창화 교수님 이미지)
        main_img_path = os.path.join(self.project_root, "image", "person_308.png")
        ...
        self.sub_img = QLabel()
        # image/person_308_qr_dbpia.png 경로로 변경 (김창화 교수님 QR 이미지)
        sub_img_path = os.path.join(self.project_root, "image", "person_308_qr_dbpia.png")
        ...
        self.qr_text_label = QLabel("")  # 텍스트 제거
        ...
        # --- 오른쪽 이미지 (클릭 가능, 308호 이미지로 변경) ---
        self.right_img = ClickableLabel()
        right_img_path = os.path.join(self.project_root, "image", "308호.png")  # 이미지 경로 308호로 변경
        ...
        if not pixmap_right.isNull():
            ...
            print(f"경고: 308호 이미지 파일을 찾을 수 없습니다: {right_img_path}")
        ...
        # 시작하자마자 MP3 재생 (308호 정보 오디오로 변경)
        self.play_info_audio("AI_SIM_mp3/info_308.mp3")  # 오디오 파일명 308로 변경
