...
from PySide2.QtWidgets import (
    QApplication, QLabel, QPushButton, QVBoxLayout,  # QPushButton 임포트 추가
    QHBoxLayout, QWidget
)
from PySide2.QtGui import QPixmap, QIcon  # QIcon 임포트 추가
from PySide2.QtCore import Qt, QSize, Signal, QTimer  # QTimer 임포트
...
class ProfessorCard(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("교수님 정보 카드")  # 윈도우 제목 설정 (변경됨)
        ...
        main_img_path = os.path.join(self.project_root, "image", "person_313.png")  # 이미지 경로 수정
        ...
        self.sub_img = QLabel()
        sub_img_path = os.path.join(self.project_root, "image", "person_313_qr_dbpia.png")  # 이미지 경로 수정
        ...
        right_img_path = os.path.join(self.project_root, "image", "302호.png")  # 이미지 경로 수정
        ...
        if not pixmap_right.isNull():
            ...
            self.right_img.setText("302호 이미지 없음")
            self.right_img.setStyleSheet("color: white; font-size: 30px;")
            print(f"경고: 302호 이미지 파일을 찾을 수 없습니다: {right_img_path}")
        ...
        # 시작하자마자 MP3 재생 (302호 정보 오디오로 변경)
        self.play_info_audio("AI_SIM_mp3/info_302.mp3")  # 오디오 파일명 302로 변경
