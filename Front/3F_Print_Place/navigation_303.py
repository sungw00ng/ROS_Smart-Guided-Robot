...
class ProfessorCard(QWidget):
    def __init__(self):
        ...
        # image/person_313.png 경로 그대로 사용 (교수님 이미지는 동일하다고 가정)
        ...
        self.sub_img = QLabel()
        # image/person_313_qr_dbpia.png 경로 그대로 사용 (QR 이미지는 동일하다고 가정)
        ...
        # --- 오른쪽 이미지 (클릭 가능, 303호 이미지로 변경) ---
        self.right_img = ClickableLabel()
        right_img_path = os.path.join(self.project_root, "image", "303호.png")  # 이미지 경로 303호로 변경
        ...
        if not pixmap_right.isNull():
            ...
            self.right_img.setText("303호 이미지 없음")
            self.right_img.setStyleSheet("color: white; font-size: 30px;")
            print(f"경고: 303호 이미지 파일을 찾을 수 없습니다: {right_img_path}")
        ...
        # 시작하자마자 MP3 재생 (303호 정보 오디오로 변경)
        self.play_info_audio("AI_SIM_mp3/info_303.mp3")  # 오디오 파일명 303으로 변경
