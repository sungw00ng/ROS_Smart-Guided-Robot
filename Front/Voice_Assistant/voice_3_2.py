#Voice_Assistant_Version 3_2
#기존 코드 훼손의 방지를 위해 따로 3_2 생성함.
#'네'입력을 받고 다른 창으로 전환.('다른 창'에는 rViz2로 매핑된 맵을 띄울예정
#혹은 opencv 카메라를 띄울수도 있음.
import sys
import pygame
...
class VoiceAssistant(QWidget):
    ...
    def process_query(self, text):
        """사용자 질문 처리 (RViz 창 표시 후 음성 입력 비활성화)"""
        text_lower = text.lower()

        if self.waiting_yes_no:
            if "그걸로 부탁해" in text_lower:
                self.play_response("followme.mp3")
                self.show_rviz_window()
                self.reset_state(with_voice_input=False)  # 음성 입력 비활성화
            elif "아니" in text_lower:
    ...
    def show_rviz_window(self):
        """RViz 시뮬레이션 창 표시 (기존 기능 변경 없이 추가)"""
        if self.rviz_window is None:
            self.rviz_window = QWidget()
            self.rviz_window.setWindowTitle("RViz 시뮬레이션")
            self.rviz_window.setGeometry(100, 100, 800, 600)
            layout = QVBoxLayout()
            label = QLabel("로봇 카메라 화면이 표시됩니다", self.rviz_window)
            label.setAlignment(Qt.AlignCenter)
            layout.addWidget(label)
            self.rviz_window.setLayout(layout)
        self.rviz_window.show()
    ...
    def reset_state(self, with_voice_input=True):
        """상태 초기화 (음성 입력 활성화 옵션 추가)"""
        self.waiting_yes_no = False
        self.hide_yes_no_buttons()
        self.hide_background_image()
        if with_voice_input:
